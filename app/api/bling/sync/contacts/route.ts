...
import { syncContactsToBling } from '@/lib/bling'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncContactsToBling(sinceDate, token.token_value)
    if (result.success) {
    ...
    const errorMessage = err instanceof Error ? err.message : 'Erro ao sincronizar clientes.'
