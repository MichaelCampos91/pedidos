...
import { syncOrdersToBling } from '@/lib/bling'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncOrdersToBling(sinceDate, '')
    if (result.success) {
    ...
    const errorMessage = err instanceof Error ? err.message : 'Erro ao sincronizar pedidos.'
