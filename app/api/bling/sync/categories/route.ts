...
import { syncCategoriesToBling } from '@/lib/bling'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncCategoriesToBling(sinceDate, token.token_value)
    if (result.success) {
    ...
    const errorMessage = err instanceof Error ? err.message : 'Erro ao sincronizar categorias.'
